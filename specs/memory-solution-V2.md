...
- **个人记忆**，不 opencode-codex-memory（LLM-Wiki） 插件或通过 opencode-mem 插件（向量RAG）集成
...
┌─────────────────────────────────────────────────────────┐
│ 个人记忆（偏好建模）                                       │
│ opencode-codex-memory 插件（LLM-Wiki） │ opencode-mem 插件（向量rag）
├─────────────────────────────────────────────────────────┤
│ 公司 / 项目 / 团队规范（静态约束）                          │
...
**路线 A：opencode-codex-memory 插件（LLM Wiki形式）**
...
- **召回哲学不同**：A 是"带目录按需取"（摘要常驻 + 模型自主深挖）、B 是"用的时候语义搜"（embedding 向量相似度匹配）
...
| 诉求 | 选 |
|---|---|
| 全自动、无需模型自觉、记忆沉淀为技能 | 路线 A |
| 记忆规模大、措辞漂移严重、接受模型下载与持续调用成本 | 路线 B |
