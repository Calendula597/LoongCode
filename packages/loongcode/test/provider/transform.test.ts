...
// LGDG ModelHub models are dynamically discovered (not from models.dev) with no
// reasoning_options, so reasoningVariants returns undefined and variants() must
// generate their thinking controls. This guards against regressions where the
// provider layer stops recomputing variants for models built without them.
describe("ProviderTransform.variants - LGDG discovered models", () => {
  const lgdgModel = (id: string) =>
    ({
      id,
      providerID: "lgdg",
      name: id,
      family: "",
      api: { id, url: "https://modelhub.lgdg.cc/aigateway/v1", npm: "@ai-sdk/openai-compatible" },
      status: "active",
      headers: {},
      options: {},
      cost: { input: 0, output: 0, cache: { read: 0, write: 0 } },
      limit: { context: 128_000, input: 128_000, output: 8192 },
      capabilities: {
        temperature: true,
        reasoning: true,
        attachment: false,
  ...
  test("GLM-5 reasoning models receive reasoningEffort variants over openai-compatible", () => {
    expect(ProviderTransform.variants(lgdgModel("glm-5.2"))).toEqual({
      high: { reasoningEffort: "high" },
      max: { reasoningEffort: "max" },
    })
  })

  test("MiniMax-M3 receives thinking toggle variants", () => {
    expect(ProviderTransform.variants(lgdgModel("minimax-m3"))).toEqual({
      none: { thinking: { type: "disabled" } },
      thinking: { thinking: { type: "adaptive" } },
    })
  })
